...
void FStreamlineRHI::SetPrePresentCommand(TFunction<void()> Lambda)
{
	// should be safe to call from other threads but we don't expect that to happen in normal usage
	ensure(!IsRunningRHIInSeparateThread() || IsInRHIThread());
	FScopeLock Lock(&PrePresentCommandCS);
	PrePresentCommand.Emplace(Lambda);
}

void FStreamlineRHI::ExecutePrePresentCommands()
{
	FScopeLock Lock(&PrePresentCommandCS);
	if (PrePresentCommand.IsSet())
	{
		PrePresentCommand->Execute();
	}
}
...
int32 FStreamlineRHI::GetMaxNumSwapchainProxies() const
{
	...
	int32 MaxNumSwapchainProxies = CVarStreamlineMaxNumSwapchainProxies.GetValueOnAnyThread();
	if (!ShouldUseSlateCallbacksForSwapchainTracking())
	{
		// Our custom swapchain proxy can't handle more than 1 instance. Not that DLSS-FG would ever use multiple proxies
		// even with slate callbacks, but this makes that explicit in code for review purposes.
		MaxNumSwapchainProxies = FMath::Min(MaxNumSwapchainProxies, 1);
	}
...
void FStreamlineRHI::PostPlatformRHICreateInit()
{
	...
	FStreamlineExtensionModule::Get().SetLoadedFeatures(LoadedFeatures);
...
{
	check(InputOutput.StreamlineTag == sl::kBufferTypeScalingOutputColor);
...
void FStreamlineRHIModule::InitializeStreamline()
{
	...
	// Registering any more SL features after this wouldn't make sense because they won't get passed to SL init
...
void FStreamlineRHIModule::StartupModule()
{
	...
	// Make sure feature registration is closed here in case SL didn't get initialized
	FStreamlineExtensionModule::Get().CloseRegistration();
	UE_LOG(LogStreamlineRHI, Log, TEXT("%s Leave"), ANSI_TO_TCHAR(__FUNCTION__));
}
